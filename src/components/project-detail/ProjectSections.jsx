...
function SectionImage({ src, alt, index = 0 }) {
  const gradient = PLACEHOLDER_GRADIENTS[index % PLACEHOLDER_GRADIENTS.length]

  if (src) {
    return (
      <div className="w-full h-[320px] md:h-[440px] rounded-[20px] overflow-hidden">
...
export default function ProjectSections({ project }) {
  ...
  // sectionImages[]: optional array of { src, alt } — one per visual break
  ...
  return (
    <section ref={sectionRef} className="px-6 pb-16">
      <div className="max-w-[1200px] mx-auto flex flex-col gap-6">
        ...
        {/* Visual break 1 */}
        <SectionImage src={imgs[0]?.src} alt={imgs[0]?.alt || project.title} index={0} />
        ...
        {presentOptional.length > 0 && (
          <>
            ...
            <SectionImage src={imgs[1]?.src} alt={imgs[1]?.alt || project.title} index={1} />
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        {(hasTakeaways || hasReflection) && (
          <SectionImage src={imgs[2]?.src} alt={imgs[2]?.alt || project.title} index={2} />
        )}
